	...
	public static class FloodNode extends RingNode {
		...
		protected void checkBeats() {
			...
			if (this.leaderID == getNodeId()) {
				...
				sendLeaderNotice();
				return;
			} else if (state == FMState.Candidate) {
			...
			if (now - lastKnownBeat > beatDelta && state != FMState.Candidate) {
				beatCounter++;
		...
		private void receiveElectionNotice(FloodMsg msg) {
			if (msg.getState() == FMState.Candidate) {
			...
			} else if (msg.getState() == FMState.Leader) {
