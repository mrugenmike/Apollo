...
public class RAFTElection implements Election {
	...
	private synchronized Management castVote(Management mgmt, boolean isNew) {
		...
		//May not be required since random timer is there in RAFT for election timeout
		LeaderElection req = mgmt.getElection();
		if (req.getExpires() <= System.currentTimeMillis()) {
		...
		/** This block of code need not be present since each node will not 
		 * redistribute the message. A node will send a message to all node - MESH network
		 * For dynamically adding nodes, we can finish that functionality later.
		 * But still keeping this code so that there is no way cycles will appear
		 */
		...
		if (req.getCandidateId() == this.nodeId) {
			...
			/** In this case we need to count the no of votes and 
			 * declare myself as the leader only if it is a majority. Also, the majority is 
			 * based on the no of connected nodes which have to be updated in some variable.
			 */
			//This statement lacks checking condition before declaring the winner.
			//notify(true, this.nodeId);
			//This statement is fine
			current.incVoteCount();
			if(current.getVoteCount() > ConnectionManager.getNumMgmtConnections()/2) {
				elb.setAction(ElectAction.DECLAREWINNER);
				//This statement is not required maybe?
				elb.setHops(mgmt.getHeader().getPathCount());
			} else {
				//Dont do anything. Just counting the votes is enough.
				//If we don't reach majority votes the election will time out
				return null;
			}
			
			ElectionManager.getInstance().setLeader(this.nodeId);
			logger.info("Node " + this.nodeId + " is declaring itself the leader");
		} else {
			/**
			 * Need to vote for the node that started the election!
			 */
			//Should check with the term instead for voting
			/*
			if (req.getCandidateId() < this.nodeId)
